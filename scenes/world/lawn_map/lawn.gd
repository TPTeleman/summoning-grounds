...
func create_grid() -> void:
	...
	var size: float = float(cell_size)
	for x in range(grid_size.x):
		for y in range(grid_size.y):
			var point := Vector2(x,y)
			var color: Color = map_image.get_pixelv(point)
			grid[point] = {
...
func get_monsters_in_lanes(lanes: Array) -> Array[MONSTER_BODY]:
	var monsters : Array[MONSTER_BODY]
	for each in enemies_node.get_children():
		var monster: MONSTER_BODY = each
...
func cell_to_pos(cell: Vector2) -> Vector2:
	var size: float = float(cell_size)
...
func pos_to_cell(pos: Vector2) -> Vector2:
	var final_pos: Vector2 = pos-grid_start
	var size: float = float(cell_size)
	return Vector2(floor(final_pos.x/size), floor(final_pos.y/size))


func get_cell_type(cell: Vector2) -> String:
	if has_cell(cell):
		return grid[cell].type
	return ""


func get_upper_summon(cell: Vector2) -> SUMMON_BODY:
	if has_cell(cell):
...
func get_upper_summon_to_attack(cell: Vector2) -> SUMMON_BODY:
	if has_cell(cell):
		var gotten_summon = null
		#print(grid[cell].summons)
		for summon in grid[cell].summons:
...
func has_place_tag(cell: Vector2, tag: String) -> bool:
	if has_cell(cell):
		for summon in grid[cell].summons:
...
func can_place_on_cell(cell: Vector2, summon: SUMMON_RES) -> bool:
	if has_cell(cell):
		if is_cell_empty(cell):
			...
			if  !summon.cell_type.has(get_cell_type(cell)):
				var placed_summon: SUMMON_BODY = get_plantable_summon(cell)
...
func get_plantable_summon(cell: Vector2) -> SUMMON_BODY:
...
func update_plants_in_cell(cell: Vector2) -> void:
...
func mouse_objects() -> void:
	var mouse_grid: Vector2 = pos_to_cell(get_global_mouse_position())
...
func _on_sun_timer_timeout() -> void:
	var time: float = level_data.sun_cooldown+randf_range(-level_data.sun_delay_range, level_data.sun_delay_range)
	sun_timer.set_wait_time(time)
	spawn_sun(Vector2(randf_range(96, 544), 0), level_data.sun_amount, 90)


func spawn_sun(pos: Vector2, sun_value: int = 25, fall_variant: int = 0) -> SUN:
	var new_sun: SUN = SUN_BODY.instantiate()
...
func on_sun_collected(node) -> void:
...
func place_summon(cell: Vector2, summon: SUMMON_RES) -> void:
	if can_place_on_cell(cell, summon):
		var new_summon: SUMMON_BODY = summon.summon_body.instantiate()
...
func spawn_monster(cell: Vector2, monster: PackedScene) -> MONSTER_BODY:
	var new_monster: MONSTER_BODY = monster.instantiate()
...
func get_monster_amount() -> int:
	var count = enemies_node.get_child_count()
	return count


func remove_one_summon(cell: Vector2) -> void:
	var summon: SUMMON_BODY = get_upper_summon(cell)
	if summon:
		#gain_sun(round(summon.summon_res.summon_cost/2))
		free_summon(cell, summon)


func free_summon(cell: Vector2, summon: SUMMON_BODY) -> void:
...
func spawn_particle(particle: PackedScene) -> TEMP_PARTICLES:
	var new_particle: TEMP_PARTICLES = particle.instantiate()
